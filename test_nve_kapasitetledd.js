/**
 * NVE TOTAL TESTER v1.1
 * * HENSIKT:
 * En stresstest-modul som går gjennom hele 'db'-objektet for å verifisere at 
 * NVE-API-et leverer forventet data for samtlige selskaper.
 * * FUNKSJONALITET:
 * 1. Sjekker tilkobling til API for hver enkelt oppføring.
 * 2. Validerer "Datafangst": Sjekker om API-et faktisk returnerer rader.
 * 3. GAP-analyse: Sjekker om nettselskapet bruker de standardiserte 
 * effekttrinnene (0, 2, 5, 10, 15, 20 kW). 
 * * * MERK: setTimeout er fjernet da HomeyScript kjører synkront/lineært i denne konteksten.
 */

async function testAlleSelskaper() {
  // Databasen som skal verifiseres (generert av Generator v5)
const db = {
   ...
   "glitre_østfold": { "org": "982974011", "fylke": "31" },
   ...
   "tensio_tn_trøndelag": { "org": "988807648", "fylke": "50" },
...
const db = {
  ...
  log(`🚀 Starter stresstest av ${selskaper.length} selskaper mot NVE...`);
  log(`------------------------------------------------------------`);

  for (const navn of selskaper) {
    const info = db[navn];
    // Konstruerer test-URL for dags dato
    const url = `https://nettleietariffer.dataplattform.nve.no/v1/NettleiePerOmradePrTimeHusholdningFritidEffekttariffer?ValgtDato=${dato}&Tariffgruppe=Husholdning&FylkeNr=${info.fylke}&OrganisasjonsNr=${info.org}`;
    ...
    try {
      ...
      if (data && data.length > 0) {
        /**
         * GAP-ANALYSE:
         * Vi henter ut alle effekttrinnene som finnes i datasettet for kl. 12:00.
         * Deretter sjekker vi om de forventede standardtrinnene (0 til 20 kW) eksisterer.
         * Hvis et selskap mangler et trinn, betyr det at de kanskje har en annen 
         * tariff-struktur (f.eks. hopper rett fra 2 til 10 kW).
         */
        const trinnFunnet = data.filter(o => o.time === 12).map(o => o.effekttrinnFraKw);
        const mangler = [0, 2, 5, 10, 15, 20].filter(v => !trinnFunnet.includes(v));
        
        const status = mangler.length === 0 ? "✅ OK" : `⚠️ GAP (Mangler: ${mangler.join(',')})`;
        
        // Logg resultat med pen formatering (padding) for lesbarhet
        log(`[${status.padEnd(12)}] ${navn.padEnd(25)} | Rader: ${data.length.toString().padStart(3)}`);
      } else {
        // API-et svarte, men fant ingen tariffer for dette fylket/orgnr
        log(`[❌ TOM     ] ${navn.padEnd(25)} | API returnerte ingen data.`);
      }
    } catch (e) {
      // Skjer ved nettverksfeil eller hvis NVE-serveren kaster en feilmelding
      log(`[🔥 FEIL    ] ${navn.padEnd(25)} | Kunne ikke koble til API.`);
    }
  }
